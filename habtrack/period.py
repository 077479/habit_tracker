"""
module period: ressource module for the tool habit_tracker (habtrack)
...
===== Classes =====
Period: represents a periodicity for a habit object

===== Dependencies =====
created and tested with "pytest 7.1.2" and "Python 3.10.5

===== Exceptions =====
TypeError: when initializing the period object, and a wrong str repr of the periodicity is given
"""
...
class Period:
    """
    class Period: represents the periodicity for a habit in a habitat_tracker tool

    ===== Attributes =====
    _periods : dict | mapping of the str representation of a periodicity to an actual functionality to 
                        return a time-period
    period : str | the str representation of the periodicity
    fct : function | the linking of the functionality from the periods dict

    ===== Methods =====
    _set_fct: maps the fct to the functionality
    _add_day: functionality that returns a daily timeperiod
    _add_week: functionality that returns a weekly timeperiod
    add_month: functionality that returns a monthly timeperiod
    __call__: calls the fct
    """

    def __init__(self, period: str) -> "Period":
        """
        ===== Parameters =====
        period : str | the str representation of the string

        ===== Returns =====
        Period Object

        ===== Exceptions =====
        TypeError: when initializing the period object, when the str representation of
    ...
    def _add_day(self, start_date: datetime.date, increment: int) -> tuple:
        """_add_day: provides the time-period generation functionality
        ...
        ===== Parameters =====
        start_date : datetime.date | represents the start_date of the time period
        icrement : int | represents the time span between the start/end date
                
        ===== Returns =====
        tuple | (start, end) => if increment<0 (end, start)
        """
    ...
    def _add_week(self, start_date: datetime.date, increment: int) -> None:
        """_add_week: provides the time-period generation functionality
    ...
    def _add_month(self, start_date: datetime.date, increment: int) -> None:
        """_add_month: provides the time-period generation functionality
    ...
    def __call__(self, start_date: datetime.date, increment: int) -> None:
        """__call__: calls the time-period functionality
