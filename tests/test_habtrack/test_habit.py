...
class TestHabit(unittest.TestCase):
    ...
    def test_habit_defect_parameters(self, name, periods):
        ...
        with pytest.raises(TypeError):
            habtrack.habit.Habit("name", periods)
    ...
    def test_habit_equality(self):
        """test for correct return"""
        hab = habtrack.habit.Habit(name="test_habit", periodicity="monthly", description="just here to test the modul")
        hab.creation_date = datetime.date(2000,1,1)
        assert self.habit_obj == hab

    
    @parameterized.parameterized.expand([("hab_1", habtrack.habit.Habit("test", "monthly")),
                                        ("int", 238),
                                        ("hab_2", habtrack.habit.Habit("john", "weekly")),
                                        ("str", "hmmw")])
