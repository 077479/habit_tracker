...
class TestStorage(unittest.TestCase):
    ...
    def test__serialize_habit_type(self):
        ...
        assert isinstance(habtrack.storage._serialize_habit(self.habit_obj), dict)

    def test__serialize_habit_dct_name(self):
        """test for the right state after call"""
        assert habtrack.storage._serialize_habit(self.habit_obj)["name"] == "test_habit"

    def test__serialize_habit_dct_period(self):
        """test for the right state after call"""
        assert habtrack.storage._serialize_habit(self.habit_obj)["periodicity"] == "monthly"

    def test__serialize_habit_dct_desc(self):
        """test for the right state after call"""
        assert habtrack.storage._serialize_habit(self.habit_obj)["description"] == "just here to test the modul"

    def test__serialize_habit_dct_creation_date(self):
        """test for the right state after call"""
        assert habtrack.storage._serialize_habit(self.habit_obj)["creation_date"] == datetime.date(2000,1,1).toordinal()
    ...
    def test__serialize_habit_dct_checkoffs(self):
        ...
        assert habtrack.storage._serialize_habit(self.habit_obj)["checkoffs"] == [730120, 730151, 730180, 730211]
    ...
    def test__deserialize(self):
        """test for the right state after call"""
        hab = habtrack.storage._deserialize_habit({
            "_HABIT": True,
            "name": "test_habit",
            "description": "just here to test the modul",
            "periodicity": "monthly",
    ...
    def test_deserialize_type(self):
        """test for the right instance"""
        assert isinstance(habtrack.storage.deserialize(), list)

    def test_deserialize_demo_type(self):
        """test for the right attribute"""
        assert habtrack.storage.deserialize(demo=True)[0].__module__ == "habtrack.habit"

    def test_deserialize_demo_len(self):
        """test for the right attribute"""
        assert len(habtrack.storage.deserialize(demo=True)) == 6

    def test_deserialize_file_source_len(self):
        """test for the right length of return"""
        habtrack.storage.serialize([self.habit_obj])
        assert len(habtrack.storage.deserialize(file_source="habit_obj")) == 1

    def test_deserialize_file_source_name(self):
        """test for the right attribute"""
        habtrack.storage.serialize([self.habit_obj])
        assert habtrack.storage.deserialize(file_source="habit_obj")[0].name == "test_habit"